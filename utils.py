...
def viz_biomap2(shp, df, dfmin, dfmax, sel_yr):

    sub_link = read_subids()
    if shp == 'AGM':
        subdf = gpd.read_file("./resources/AGM_pastures.shp")
    if shp == 'TGM':
        subdf = gpd.read_file("./resources/CPER_AGM_TGM_PasturesOnly.shp")        
    subdf = subdf.merge(sub_link, on='Past_Name_')
...
def get_corr_plot(df, v1s, v2s):
    ...
    for i, j, k in zip(v1s, v2s, range(len(v1s))):
        ...
        v2_df = df.loc[:, j]
        fig.add_trace(go.Scatter(
            x=v1_df, y=v2_df, name='Reach {}'.format(i),
            mode='markers',
    ...
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='lightgray', title='Variables on X-axis')
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='lightgray', title='Variables on Y-axis')
    fig.update_layout(legend=dict(
        yanchor="top",
        y=1.0,
        xanchor="center",
